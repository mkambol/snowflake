    ...
    if ( row != null && first ) {
      ...
      if ( meta.isSpecifyFields() && meta.getDataType().equals(
        ...
        for ( int i = 0; i < meta.getSnowflakeBulkLoaderFields().length; i++ ) {
          ...
          data.fieldnrs
            .put( meta.getSnowflakeBulkLoaderFields()[ i ].getTableField().toUpperCase(), streamFieldLocation );
        }
      ...
      if ( meta.getDataTypeId() == SnowflakeBulkLoaderMeta.DATA_TYPE_CSV && !meta.isSpecifyFields() ) {
        ...
        for ( int i = 0; i < data.dbFields.size(); i++ ) {
          if ( data.dbFields.get( i ) != null ) {
            ...
            } else if ( field[ 1 ].toUpperCase().startsWith( "NUMBER" ) || field[ 1 ].toUpperCase()
              .startsWith( "FLOAT" ) ) {
            ...
            if ( fieldIndex >= 0 ) {
              ...
              throw new KettleException(
                "Error column mismatch: Database field " + data.dbFields.get( i )[ 0 ] + " not found on stream." );
            }
